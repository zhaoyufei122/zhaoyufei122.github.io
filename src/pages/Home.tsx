  ...
  return (
    <div className="pt-24 pb-16 px-6 max-w-5xl mx-auto">
      ...
      <motion.section 
        ...
        className="py-20"
      >
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 font-mono text-xs mb-8">
        ...
        <h1 className="text-5xl md:text-7xl font-bold text-zinc-100 tracking-tight mb-6">
          Building <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-cyan-500">Intelligent</span> <br />
          Physical Systems.
        </h1>
        ...
        <div className="flex flex-wrap items-center gap-4">
          <Link 
            to="/projects"
            className="px-6 py-3 rounded-lg bg-zinc-100 text-zinc-900 font-medium hover:bg-white transition-colors flex items-center gap-2"
          >
            View Projects <ChevronRight className="w-4 h-4" />
          </Link>
          <Link 
            to="/contact"
            className="px-6 py-3 rounded-lg bg-zinc-900 text-zinc-100 border border-zinc-800 font-medium hover:bg-zinc-800 transition-colors"
          >
            Contact Me
          </Link>
        </div>
      ...
      <motion.section 
        ...
        viewport={{ once: true }}
        transition={{ duration: 0.5 }}
        className="py-16 border-t border-zinc-800/50"
      >
      ...
      <motion.section 
        ...
        className="py-16 border-t border-zinc-800/50"
      >
        <h2 className="text-2xl font-bold text-zinc-100 mb-12 flex items-center gap-3">
          ...
          {personalInfo.experience.map((exp, index) => (
            <div key={index} className="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-8">
              <div className="md:col-span-1">
                <p className="font-mono text-sm text-emerald-400 sticky top-24">{exp.period}</p>
              </div>
              <div className="md:col-span-3">
                <h3 className="text-xl font-bold text-zinc-100">{exp.title}</h3>
                <p className="text-zinc-400 font-medium mb-4">{exp.organization}</p>
        ...
        <div className="space-y-8">
          {personalInfo.education.map((edu, index) => (
            <div key={index} className="relative pl-8 before:absolute before:left-0 before:top-2 before:bottom-0 before:w-px before:bg-zinc-800">
              <div className="absolute left-[-4px] top-2 w-2 h-2 rounded-full bg-emerald-500 ring-4 ring-zinc-950" />
              <div className="flex flex-col md:flex-row md:items-center justify-between mb-2">
                <h3 className="text-lg font-semibold text-zinc-100">{edu.institution}</h3>
                <span className="font-mono text-sm text-emerald-400">{edu.period}</span>
              </div>
              <p className="text-zinc-300 font-medium mb-2">{edu.degree}</p>
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {personalInfo.achievements.map((achievement, index) => (
            <div key={index} className="flex items-center gap-4 p-4 rounded-xl bg-zinc-900/30 border border-zinc-800/50">
...
function SkillCard({ icon: Icon, title, skills }: { icon: any, title: string, skills: string[] }) {
  return (
    <div className="p-6 rounded-2xl bg-zinc-900/50 border border-zinc-800/50 hover:border-emerald-500/30 transition-colors group">
      ...
      <h3 className="text-zinc-100 font-semibold mb-4">{title}</h3>
      <div className="flex flex-wrap gap-2">
        {skills.map((skill) => (
          <span key={skill} className="px-2.5 py-1 rounded-md bg-zinc-800/50 text-zinc-300 text-xs font-mono border border-zinc-700/50">
            {skill}
          </span>
        ))}
      </div>
