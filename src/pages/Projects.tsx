...
export default function Projects() {
  ...
  return (
    <div className="pt-24 pb-16 px-6 max-w-5xl mx-auto">
      ...
      >
        <h1 className="text-4xl font-bold text-zinc-100 mb-4">Projects & Repositories</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {personalInfo.githubProjects
                ...
                .map((project, index) => (
                  <div 
                    key={index} 
                    className="flex flex-col p-6 rounded-2xl bg-zinc-900/30 border border-zinc-800/50 hover:border-emerald-500/50 hover:bg-zinc-900/80 transition-all group"
                  >
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        {project.demoUrl && (
                          <a 
                            ...
                            target="_blank" 
                            rel="noreferrer" 
                            className="flex items-center gap-1.5 text-xs font-mono text-emerald-400 hover:text-emerald-300 transition-colors bg-emerald-500/10 px-2.5 py-1.5 rounded-md border border-emerald-500/20"
                            title="Play Demo / View Live"
                          >
                            <Play className="w-3 h-3 fill-emerald-400" /> PLAY
                          </a>
                        )}
                        <a 
                    ...
                    <div className="flex flex-wrap gap-2 mt-auto">
                      {project.techStack.map(tech => (
                        <span 
                          key={tech} 
                          className="px-2.5 py-1 rounded bg-zinc-800/50 text-zinc-300 text-xs font-mono border border-zinc-700/50"
                        >
                          {tech}
                        </span>
                      ))}
